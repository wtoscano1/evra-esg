...
export default function ESGEvraSite() {
  ...
  const tAll = {
    ...
    it: {
      ...
      org: { title: "Organizzazione", people: [ { name: "William Toscano", role: "Operazioni", note: "GM fine dining a Beverly Hills." }, { name: "David Toscano", role: "Sourcing & ESG", note: "Rete diretta di produttori italiani." } ] },
      clients: { title: "Partner & Produttori", blurb: "Selezione di produttori italiani e partner logistici." },
      projects: { title: "Progetti", cards: [ { h: "Monti", p: "Carni alla brace; centro di Ginevra.", tag: "2026" }, { h: "Mare", p: "Seafood mediterraneo.", tag: "Fase 2" }, { h: "Farina", p: "Pizza & pasta.", tag: "Fase 3" } ] },
      geo: { title: "Espansione & KPI", bullets: [ "Da 1 a 3 ristoranti in 5 anni", "Vantaggio sul food cost", "Adozione tracciabilità e riduzione sprechi", "Volume pre‑acquisti e retention" ] },
    ...
    fr: {
      ...
      advantages: { title: "Avantage Concurrentiel", cols: [ { h: "Problème", p: "Coûts élevés, provenance opaque, logistique fragmentée, scope‑3." }, { h: "Solution", p: "Réseau de producteurs, pré‑achats, traçabilité vérifiée, chaîne du froid optimisée." }, { h: "Résultat", p: "Meilleures marges, moins de pertes, ESG mesurable, expérience supérieure." } ] },
      services: { title: "Services", items: [ { h: "Approvisionnement & Pré‑achat", p: "Lots saisonniers financés à l’avance." }, { h: "Traçabilité", p: "QR par article: producteur, lot, trajet, certifications." }, { h: "Opérations", p: "Monti • Mare • Farina sur une base commune." }, { h: "Distribution", p: "Dernier km à température contrôlée." } ] },
      org: { title: "Organisation", people: [ { name: "William Toscano", role: "Opérations", note: "Direction de restaurants haut de gamme." }, { name: "David Toscano", role: "Sourcing & ESG", note: "Réseau direct de producteurs italiens." } ] },
      clients: { title: "Partenaires & Producteurs", blurb: "Producteurs italiens et partenaires logistiques." },
      projects: { title: "Projets", cards: [ { h: "Monti", p: "Viandes au feu de bois; Genève centre.", tag: "2026" }, { h: "Mare", p: "Produits de la mer méditerranéens.", tag: "Phase 2" }, { h: "Farina", p: "Pizza & pâtes.", tag: "Phase 3" } ] },
      geo: { title: "Expansion & KPI", bullets: [ "1 → 3 restaurants en 5 ans", "Avantage coût matière", "Adoption traçabilité et réduction des déchets", "Volume de pré‑achat et rétention" ] },
  ...
  // Files to render on the Documents table
  const docsManifest = [
    { title: "Investor Deck (PDF)", path: "/deck/ESG_Evra_Investor_Deck.pdf", type: "PDF", lang: "EN", date: "2025-11-07" },
    { title: "Executive Summary", path: "/docs/Executive_Summary_EN.pdf", type: "PDF", lang: "EN", date: "2025-11-07" },
    { title: "Synthèse Exécutive", path: "/docs/Resume_Executif_FR.pdf", type: "PDF", lang: "FR", date: "2025-11-07" },
    { title: "Sintesi Esecutiva", path: "/docs/Sintesi_Esecutiva_IT.pdf", type: "PDF", lang: "IT", date: "2025-11-07" },
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      {/* Header */}
      <header className={`sticky top-0 z-30 backdrop-blur bg-white/80 border-b border-neutral-200 transition-shadow ${shadow ? "shadow-sm" : ""}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-neutral-900 to-neutral-700" aria-hidden />
          ...
          <nav className="hidden md:flex gap-6 text-sm">
            ...
            <NavLink href="#advantages" label={t.nav.pillars} />
            <NavLink href="#services" label={t.nav.services} />
            <NavLink href="#org" label={t.nav.org} />
            <NavLink href="#clients" label={t.nav.clients} />
            <NavLink href="#projects" label={t.nav.projects} />
            <NavLink href="#geo" label={t.nav.geo} />
            <NavLink href="#documents" label={t.nav.documents} />
            <NavLink href="#contact" label={t.nav.contact} />
            <NavLink href="#imprint" label={t.nav.imprint} />
          </nav>

          {/* Actions */}
          <div className="flex items-center gap-2">
            <LangSwitcher lang={lang} setLang={setLang} />
            <a href="#contact" className="hidden sm:inline-flex rounded-xl bg-neutral-900 text-white px-3 py-1.5 text-sm hover:opacity-90">{t.hero.cta1}</a>
            <button onClick={() => setMobileOpen((v) => !v)} className="md:hidden rounded-lg border border-neutral-300 px-2 py-1 text-sm">Menu</button>
          </div>
        </div>
  ...
  <div className="md:hidden border-t border-neutral-200 bg-white">
    <div className="mx-auto max-w-7xl px-4 py-3 grid grid-cols-2 gap-3 text-sm">
      {[
        ...
        <a key={href} href={href} className="py-2 rounded-lg hover:bg-neutral-100 px-2">
          {label}
        </a>
      ))}
    </div>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10 bg-gradient-to-b from-neutral-100 via-white to-white" />
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          <p className="text-xs tracking-widest uppercase text-neutral-500">{t.hero.kicker}</p>
          <h1 className="mt-3 text-4xl sm:text-6xl font-bold tracking-tight">{t.hero.title}</h1>
          <p className="mt-4 max-w-2xl text-lg text-neutral-700">{t.hero.subtitle}</p>
          <div className="mt-8 flex gap-3">
            <a href="#contact" className="rounded-2xl bg-neutral-900 text-white px-5 py-3 text-sm hover:opacity-90">{t.hero.cta1}</a>
            <a href="#projects" className="rounded-2xl border border-neutral-300 px-5 py-3 text-sm hover:bg-neutral-100">{t.hero.cta2}</a>
          </div>
        </div>
      </section>
      ...
      {/* Advantages: Problem → Solution → Result */}
      <Section id="advantages" title={t.advantages.title}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {t.advantages.cols.map((c, i) => (
            <div key={i} className="rounded-2xl border border-neutral-200 p-6 bg-white shadow-sm hover:shadow-md transition-shadow">
              <div className="text-xs uppercase tracking-widest text-neutral-500">{c.h}</div>
      ...
      <Section id="services" title={t.services.title}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.services.items.map((s, i) => (
            <div key={i} className="rounded-2xl border border-neutral-200 p-6 bg-white shadow-sm hover:shadow-md transition-shadow">
              <div className="h-8 w-8 rounded-xl bg-neutral-200" aria-hidden />
              <h3 className="mt-3 font-semibold">{s.h}</h3>
              <p className="mt-2 text-sm text-neutral-700">{s.p}</p>
            </div>
          ))}
        </div>
      </Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {t.org.people.map((p, i) => (
            <div key={i} className="rounded-2xl border border-neutral-200 p-6 bg-white shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="mt-3 text-sm text-neutral-700">{p.note}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section id="clients" title={t.clients.title} blurb={t.clients.blurb}>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="h-12 rounded-xl bg-neutral-200" aria-label="logo placeholder" />
          ))}
        </div>
      </Section>
      ...
      <Section id="documents" title={t.docs.title} blurb={t.docs.hint}>
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          ...
          <div className="text-xs text-neutral-500">{filteredDocs.length} file(s)</div>
        </div>
        <div className="mt-6" />
      ...
      <Section id="contact" title={t.contact.title} blurb={t.contact.blurb}>
        <form
          className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white p-6 rounded-2xl border border-neutral-200 shadow-sm"
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <label className="block">
            ...
            <input required className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900/10" placeholder="Jane Doe" />
          </label>
          <label className="block">
            <span className="text-sm">{t.contact.fields.email}</span>
            <input type="email" required className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900/10" placeholder="jane@domain.com" />
          </label>
          <label className="block sm:col-span-2">
            <span className="text-sm">{t.contact.fields.msg}</span>
            <textarea required rows={5} className="mt-1 w-full rounded-xl border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-neutral-900/10" placeholder="Tell us a bit about your interest…" />
          </label>
          <div className="sm:col-span-2">
            <button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-neutral-900 text-white px-5 py-3 text-sm hover:opacity-90">
      ...
      {/* Imprint */}
      <section id="imprint" className="bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
      ...
      <footer className="border-t border-neutral-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="text-sm text-neutral-600">{t.footer.left}</div>
          <div className="text-sm text-neutral-600">{t.footer.right}</div>
        </div>
      </footer>
    </div>
  );
...
function NavLink({ href, label }) {
  return (
    <a href={href} className="relative py-1 hover:opacity-100 text-neutral-700">
      <span className="after:content-[''] after:absolute after:left-0 after:-bottom-0.5 after:h-[2px] after:w-0 after:bg-neutral-900 hover:after:w-full after:transition-all">{label}</span>
    </a>
  );
}
  ...
  return (
    <section id={id} className="bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        ...
        {blurb ? <p className="mt-2 text-neutral-700 max-w-3xl">{blurb}</p> : null}
...
function StatCard({ h, p }) {
  return (
    <div className="rounded-2xl border border-neutral-200 p-6 bg-white shadow-sm">
      <div className="text-xs uppercase tracking-widest text-neutral-500">KPI</div>
      <h3 className="mt-1 font-semibold">{h}</h3>
      <p className="mt-2 text-sm text-neutral-700">{p}</p>
    </div>
  );
}
...
function Select({ label, value, onChange, options }) {
  return (
    <label className="text-sm inline-flex items-center gap-2">
      ...
      <select
        className="rounded-lg border border-neutral-300 px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-neutral-900/10"
        value={value}
          ...
          {files.map((f, idx) => (
            <tr key={idx} className="border-b border-neutral-100 hover:bg-neutral-50">
              ...
              <td className="py-2 pr-4">
                <a className="inline-flex items-center gap-1 underline" href={f.path} target="_blank" rel="noreferrer">Download</a>
              </td>
            </tr>
          ))}
        </tbody>
